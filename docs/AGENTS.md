...
| `enabled` | bool | true | Master switch for this chat |
...
| `/agent new <name> [display]` | Create agent (copies prompts) |
| `/agent delete <name>` | Delete non-default agent |
...
The AI can read and modify configs at runtime:

| Tool | Description |
|------|-------------|
| `cfg_get_agent_config` | Show current agent config |
| `cfg_update_agent_config(key, value)` | Modify agent config |
| `cfg_get_chat_config` | Show current chat config |
| `cfg_update_chat_config(key, value)` | Modify chat config |
| `cfg_add_allowed_sender(id)` | Allow sender in current chat |
| `cfg_remove_allowed_sender(id)` | Remove sender from chat |
| `cfg_list_allowed_senders` | List allowed senders |
